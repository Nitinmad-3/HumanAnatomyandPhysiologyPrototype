...
<head>
  <meta charset="UTF-8">
  <title>Assessment: Identification of Appendicular Bones</title>
  <style>
...
<body>
  <header>
    <h1>Assessment: Identification of Appendicular Bones</h1>
    <nav>
  ...
  <script>
    ...
    const questions = [
      ...
      {
        type: "mcq",
        text: "1. The largest bone of the hip is:",
        options: [
          "Ischium",
          "Ilium",
          "Pubis",
      ...
      {
        type: "mcq",
        text: "2. How many bones are found in each human arm?",
        options: [
          "30",
          "26",
          "14",
      ...
      {
        type: "mcq",
        text: "3. The scapula is also known as the:",
        options: [
          "Collarbone",
          "Hip bone",
          "Shoulder blade",
      ...
      {
        type: "mcq",
        text: "4. Which bone forms the knee cap?",
        options: [
          "Tibia",
          "Patella",
          "Fibula",
      ...
      // One Word/Short Answer Questions (Dropdown) //
      {
        type: "dropdown",
        text: "5. Name the bones that make up the pectoral girdle.",
        options: [
          "Radius and Ulna",
      ...
      {
        type: "dropdown",
        text: "6. What is the longest bone in the human body?",
        options: [
          "Tibia",
          "Humerus",
          "Femur",
      ...
      {
        type: "dropdown",
        text: "7. Which bones form the palm of the hand?",
        options: [
          "Carpals",
          "Phalanges",
          "Metacarpals",
      ...
      {
        type: "dropdown",
        text: "8. How many phalanges are there in one human foot?",
        options: [
          "10",
          "14",
          "26",
      ...
      // --- End One Word/Short Answer Questions (Dropdown) --- //

      // Application/Diagram-Based Questions (Image-Labels) //
      {
        type: "image-labels",
        text: "9. Identify the labelled bones in the diagram below:",
        image: "../assets/images/experiment-3/Assessment/Assessment exptno7appendicular.png",
        labels: [
          {
            name: "1",
            answer: "Phalanges",
            options: [
              "Phalanges", "Scapula", "Ulna", "Carpals", "Tarsals", "Patella", "Tibia"
            ]
          },
          {
            name: "2",
      ...
      // --- End Application/Diagram-Based Questions (Image-Labels) --- //
    ];
    // --- End Assessment Questions Data --- //

    // Shuffle options for MCQ/Dropdowns and labels //
    questions.forEach(q => {
      if (q.options) shuffleArray(q.options);
      if (q.labels) q.labels.forEach(lab => shuffleArray(lab.options));
    });
    // --- End Shuffle options for MCQ/Dropdowns and labels --- //
    ...
    document.getElementById("assessmentForm").onsubmit = async function(e) {
      ...
      for(let i=0; i<questions.length; i++) {
        if(questions[i].type === "mcq" || questions[i].type === "dropdown") {
          ...
          questions[i].labels.forEach((label, lid) => {
            maxMarks += 1;
            if(input[lid] && input[lid] === label.answer) marks++;
          });
        }
      }
      // --- End Calculate marks (auto-graded) --- //

      let scoreSummary = `${marks}/${maxMarks}`;
      ...
      data.append('experiment', "Identification of Appendicular Bones");
      answers.forEach((ans,i) => data.append('q'+i, ans));
      data.append('marks', marks);
      data.append('scoreSummary', scoreSummary);
      // --- End Prepare FormData for Apps Script (must match sheet headers) --- //
      ...
      // --- End Your deployed Apps Script endpoint (provided by you) --- //
      ...
      document.getElementById("resultMsg").innerHTML = `Quiz submitted! You scored ${marks} out of ${maxMarks} on the auto-graded questions.`;

      // --- Feedback for all answers (correct and wrong) --- //
      let feedbackHtml = "<hr><b>Review your answers:</b><br>";
      for (let i = 0; i < questions.length; i++) {
        ...
        if (questions[i].type === "mcq" || questions[i].type === "dropdown") {
          ...
          feedbackHtml += `<div style="margin-bottom:1em;">
            <b>Q${i+1}:</b> ${questions[i].text}<br>
            <span class="${isCorrect ? 'feedback-correct' : 'feedback-wrong'}">
              Your answer: <i>${userAns || "(blank)"}</i> – <b>${isCorrect ? "Correct" : "Wrong"}</b>
            </span><br>
            ${!isCorrect ? `<span class="feedback-correct">Correct answer: <b>${correctAns}</b></span>` : ""}
          </div>`;
        } else if (questions[i].type === "image-labels") {
          let userLabels = userAns.split(";").map(s => s.trim());
          let subFeedback = "";
          questions[i].labels.forEach((label, lid) => {
            let labelAns = userLabels[lid] || "";
            let labelCorrect = labelAns === label.answer;
            subFeedback += `<br>
              <span class="${labelCorrect ? 'feedback-correct' : 'feedback-wrong'}">
                <b>Label ${label.name}:</b> Your answer: <i>${labelAns || "(blank)"}</i> – <b>${labelCorrect ? "Correct" : "Wrong"}</b>
              </span>
              ${!labelCorrect ? `<span class="feedback-correct"> Correct: ${label.answer}</span>` : ""}
            `;
          });
          feedbackHtml += `<div style="margin-bottom:1em;">
            <b>Q${i+1}:</b> ${questions[i].text} ${subFeedback}
          </div>`;
        }
      }
      document.getElementById("resultMsg").innerHTML += feedbackHtml;
      // --- End Feedback for all answers (correct and wrong) --- //
